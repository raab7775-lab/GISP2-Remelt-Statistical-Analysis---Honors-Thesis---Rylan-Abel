...
Thesis Abstract:
The Greenland Ice Sheet Project 2 (GISP2) is an ice core drilled within the Greenland Ice Sheet that has produced several foundational datasets for the study of paleoclimate. The core’s measured stable isotope record of δ18O and δD is one of these foundational datasets, completed in 1997 for the measurement of δ18O and in 2004 for δD, using discrete Isotope Ratio Mass Spectroscopy analysis. Beginning in 2024, there has been an effort to reanalyze the GISP2 ice core for δ18O and δD at a higher resolution, completed by the INSTAAR Stable Isotope Laboratory with Calving Ring-down Laser Spectroscopy Continuous Flow Analysis. Using these three datasets, an analysis was completed that aimed to downsample an interval from the new dataset to the scale of the older datasets to complete a statistical analysis. This was accomplished to discover any apparent differences in the datasets due to storage or as a result of using two different measurement instruments. The results of this are the detection of two differences between the old and new datasets. The first is a systematic offset between the datasets. Within the δ18O measurement campaign, the 2025 dataset was discovered to be isotopically heavier than the 1997 dataset, and for the δD measurement campaign, the 2025 dataset was isotopically lighter than the 2004 dataset. These differences were speculated to be caused by minimal core alteration with storage and standard contamination. The second difference is an offset within the depth scale of the core, where the measured values of the 2025 dataset match most closely with the 2004 dataset when every point of the 2025 dataset is moved 0.02 m down core before downsampling. These differences were speculated to be a result of borehole inclination upon drilling and layer tilt. Both offsets are within the error of the δ18O and δD standard systems and original GISP2 chronology, meaning they are minimal enough to declare that the two datasets can be treated as equivalent in future analysis. Thus, a core that has been in store for 30 years can successfully be remelted on a CRDS-CFA system and can still come to the same results as the IRMS system.
